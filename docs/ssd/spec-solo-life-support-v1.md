...
4. 買い物リスト自動生成
   ...
   - 各候補には推定根拠として「最終購入日」「推定消費間隔」を表示する。
   - ユーザーが提案を編集して最終リストを確定できる。
   - 初期データ不足時は推定精度が低い前提を明示し、手動編集を前提にしたUIを提供する。
   - 最終リスト確定前に、予算残と候補金額合計の整合チェックを行う。
...
- [ ] 買い物候補が「定期購入品 / 在庫切れ推定品 / 予算内候補」の3区分で生成・表示される。
- [ ] 各候補に推定根拠（最終購入日・推定消費間隔）が表示される。
- [ ] 初期データ不足時は「推定精度低下」の注意表示が出て、手動編集を経て確定できる。
- [ ] 予算残と候補金額合計の整合チェック結果（不足/超過）が確定前に表示される。
- [ ] 月次レポートでカテゴリ別内訳と前月比が確認できる。
- [ ] 月次レポートで総支出・カテゴリ別割合・前月差分が確認できる。
- [ ] 月次レポートで収支差額と残高推移が確認できる。
- [ ] 予算比（使用率）から月次レポート詳細へ遷移できる。
- [ ] 月次レポートでルールベースの改善アクション候補が表示される。
- [ ] 1か月分表示の応答性能目標（2秒以内）の計測手順が定義されている。
...
## 次のステップ（要件定義後）
- 要件をこの版で凍結し、`docs/ssd/task-solo-life-support-v1.md` のスプリント計画に沿ってWeb版MVP実装へ進む。
- 仕様変更が必要になった場合は、実装チケット起票時に差分理由と影響範囲を明記してから反映する。
